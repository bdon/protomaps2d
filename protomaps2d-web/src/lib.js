...
(function() {
    ...
    __exports.init = async function(wasm_path, font_faces) {
        ...
        for (var f of font_faces) {
            var font = new FontFace(f[0],`url(${f[1]})`,{weight:'100 900'})
            await font.load()
    ...
    __exports.Layer = L.GridLayer.extend({
        initialize: function(tile_url,options) {
            ...
            options.style = options.style || {labels:true,name:"name",font:"Inter",text_scale:ratio,bg_color:"#ffffff"} 
        ...
        rerender: function() {
            for (let [k,v] of Object.entries(this._tiles)) {
                ...
                const tile = v.el
                var result = wasm_bindgen.wasm_render_tile(tile.id,tile.arr,coords.z,tile.total,tile.dx,tile.dy,this.style)
            }
        },
        ...
        createTile: function(coords,done){
            ...
            fetch(tile_url).then(resp => {
                ...
                tile.arr = arr
                var result = wasm_bindgen.wasm_render_tile(tile.id,arr,coords.z,tile.total,tile.dx,tile.dy,this.style)
                done(error,tile)
            })
            return tile
        },
    ...
    protomaps2d = Object.assign({}, __exports);
})();